...
class RLAgent:
	...
	def get_action(self, state, last_action=None, ipm_predict=None, with_noise=True):
		"""
		Get action 
		...
		if with_noise:
			...
			action = self.ddpg_per.act_with_noise(
				inputs, noise_param=self.noise_param, mode=self.noise_mode
			)
	...
	def _bcm_objective(self, x, u):
		return -np.dot(u, x[:self.action_dim]) + self.cost_bps*np.sum(x[self.action_dim+1:])
...
class Actor(nn.Module):
	def __init__(self, state_dim, action_dim, ipm_dim, k, params, last_action):
		...
		if last_action:
			# with last action
			fc.append(nn.Linear(input_dim+action_dim+ipm_dim, params_fc[0])) 
		else:
			# without last action
			fc.append(nn.Linear(input_dim+ipm_dim, params_fc[0])) 
	...
	def forward(self, state, last_action=None, ipm=None):
		...
		if ipm is not None:
			state = torch.cat([state, ipm], dim=1) # concatenate LSTM outputs with price prediction


		if self.dropout is not None:
			a = self.dropout(state) # dropout after first fully connected layer

		n = len(self.fc)
		a = torch.relu(self.fc[0](a))

		for i in range(1,n-1):
			a = torch.relu(self.fc[i](a))
...
class Critic(nn.Module):
	def __init__(self, state_dim, action_dim, ipm_dim, k, params, last_action):
		...
		if last_action:
			# with last_action
			fc.append(nn.Linear(input_dim+2*action_dim+ipm_dim, params_fc[0])) 
		else:
			# without last_action
			fc.append(nn.Linear(input_dim+action_dim+ipm_dim, params_fc[0])) 
	...
	def forward(self, state, action, last_action=None, ipm=None):
		...
		if ipm is not None:
			a = torch.cat([state, ipm, action], dim=1) # concatenate with action
		else:
			a = torch.cat([state, action], dim=1) # concatenate with action
